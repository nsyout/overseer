...
cmd_verify() {
	...
	tasks)
		...
		if echo "$interactive" | grep -q "button.*P[0-2]"; then
			local task_count=$(echo "$interactive" | grep -c "P[0-2]" || echo 0)
...
cmd_flow() {
	...
	filter)
		...
		for filter_ref in "@e1" "@e2" "@e3" "@e4" "@e5"; do
			...
			local task_count=$(echo "$snap" | grep -c "button.*P[0-2]" || echo 0)
	...
	select)
		...
		local task_ref=$(echo "$interactive" | grep "button.*P[0-2]" | head -1 | sed -n 's/.*\[ref=\(e[0-9]*\)\].*/\1/p')
		task_ref="@$task_ref"
	...
	filter-select)
		...
		# Get fresh snapshot
		local snap=$(npx agent-browser snapshot -i 2>/dev/null)
		local task_ref=$(echo "$snap" | grep "button.*Blocked.*P[0-2]" | head -1 | grep -o 'ref=e[0-9]*' | sed 's/ref=/@/')
